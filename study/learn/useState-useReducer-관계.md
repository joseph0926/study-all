...
| 대상 토픽 | 관계 | 근거 |
|-----------|------|------|
| `study/src/useRef-내부구현-패턴.md` | 동일 hook 인프라 | useState/useReducer/useRef 모두 `mountWorkInProgressHook()`으로 hook 노드를 생성하고 `memoizedState`에 값을 저장하는 동일 구조 |
| `study/learn/React-리렌더링-트리거.md` | 인과 관계 | useState의 dispatch(`dispatchSetState`)와 useReducer의 dispatch(`dispatchReducerAction`)가 리렌더링을 트리거하는 경로 |
| `study/learn/React-리렌더링-비용과-메모이제이션.md` | 상위 메커니즘 | useCallback의 참조 동일성 유지 → React.memo의 얕은 비교에서 bail out → 리렌더링 비용 절감 체인 |
