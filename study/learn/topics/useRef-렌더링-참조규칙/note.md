...
이전 학습 `study/learn/topics/React-리렌더링-비용과-메모이제이션/note.md`에서 React Compiler의 `Refs` 카테고리를 다룬 부분이 직접 관련됩니다:
...
| 대상 토픽 | 관계 | 근거 |
|-----------|------|------|
| `study/learn/topics/React-리렌더링-비용과-메모이제이션/note.md` | 상위 메커니즘 | React Compiler가 `ref.current` 접근을 `Refs` 카테고리로 분류해 자동 메모이제이션에서 제외하는 이유가 본 토픽의 핵심 — 렌더 중 ref 접근은 순수하지 않은 부수효과이므로 캐시를 안전하게 재사용 불가 (line 721) |
| `study/learn/topics/React-리렌더링-트리거/note.md` | 전제 지식 | `scheduleUpdateOnFiber` → 리렌더링 트리거 경로를 useRef가 의도적으로 갖지 않는다는 것이 본 토픽의 출발점 |
