...
| 대상 토픽 | 관계 | 근거 |
|-----------|------|------|
| `study/learn/topics/React-context-전달과-상태경계/note.md` | 역할 분리 | `Context`는 전달자, 외부 store는 값의 소유자라는 구분이 이번 판단 기준의 출발점이다. |
| `study/learn/topics/React-리렌더링-트리거/note.md` | 인과 관계 | external store의 subscribe callback은 React가 다시 렌더를 여는 입구가 되며, transition fallback 설명도 렌더 트리거와 일관성 보장 문제로 연결된다. |
| `study/learn/topics/useRef-렌더링-참조규칙/note.md` | 문제-해결 관계 | `ref.current`처럼 외부 mutable 값을 렌더에 직접 읽으면 tearing 위험이 있고, `useSyncExternalStore`의 commit 직전 재확인은 그 위험을 막기 위한 보호 장치다. |
