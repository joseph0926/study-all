...
- <https://react.dev/learn/passing-data-deeply-with-context> (확인일: 2026-03-08)
- <https://react.dev/reference/react/useContext> (확인일: 2026-03-08)
- `useRef` 관련 연결 문서: `study/learn/useRef-렌더링-참조규칙.md`
...
- <https://react.dev/reference/react/useContext> (확인일: 2026-03-08)
- <https://react.dev/learn/scaling-up-with-reducer-and-context> (확인일: 2026-03-08)
- 보강 연결: `study/learn/React-리렌더링-트리거.md`
...
| 대상 토픽 | 관계 | 근거 |
|-----------|------|------|
| `study/learn/useState-useReducer-관계.md` | 역할 분리 | 상태를 실제로 관리하는 도구와, 그 결과를 전달하는 Context를 구분해서 이해해야 이번 주제가 정확해진다. reducer/state는 관리, Context는 전달이라는 분업이 반복해서 등장했다. |
| `study/learn/React-리렌더링-트리거.md` | 렌더 영향 경계 | Context 전달 자체보다 중요한 것은 Provider value 변경 시 어떤 consumer들이 영향을 받는지다. 전달 메커니즘과 리렌더 경계를 분리해 보자는 이번 대화의 문제의식과 직접 연결된다. |
| `study/learn/useRef-렌더링-참조규칙.md` | 보완 관계 | `ref.current`는 변경돼도 렌더를 일으키지 않으므로, Context가 전달하는 값의 원천이 `ref`일 때는 반응형 공유와 비반응형 공유를 구분해서 설명해야 한다. |
