...
| 대상 토픽 | 관계 | 근거 |
|-----------|------|------|
| `study/learn/FORGE-React-렌더링-파이프라인.md` | 상위 프레임워크 | "서술-실행 분리" 원칙(useEffect는 fn을 서술로 저장, 실행은 passive effects), "경계선은 의도적" 원칙(effect 결과를 React가 관리하지 않는 것은 설계 결정) |
| `study/learn/React-리렌더링-트리거.md` | 인과 관계 | scheduleUpdateOnFiber로 render 트리거 → render 결과 Effect 생성 → passive effects에서 실행 → effect 내 setState가 다시 scheduleUpdateOnFiber 호출 |
| `study/learn/React-데이터-페칭-불관여.md` | 동일 원칙 적용 | React가 데이터 페칭을 하지 않는 이유와, effect 결과를 관리하지 않는 이유는 같은 "경계선은 의도적" 원칙 |
