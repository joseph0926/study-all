...
### 연결
- `study/learn/브라우저-JS-실행-원리.md`와 연결됩니다. 메인 스레드, 이벤트 루프, 렌더링 파이프라인을 알아야 LCP/INP의 원인을 해석할 수 있습니다.
- `study/learn/React-리렌더링-비용과-메모이제이션.md`와 연결됩니다. React 비용은 체감 지표 자체가 아니라 원인 후보라는 점이 이어집니다.
...
| 대상 토픽 | 관계 | 근거 |
|-----------|------|------|
| `study/learn/브라우저-JS-실행-원리.md` | 상위 메커니즘 | LCP/INP 해석의 핵심인 메인 스레드, 이벤트 루프, 렌더링 파이프라인이 여기서 정리돼 있습니다. |
| `study/learn/React-리렌더링-비용과-메모이제이션.md` | 원인-진단 관계 | React render/commit 비용은 체감 지표 자체가 아니라 원인 후보라는 점이 이어집니다. |
| `study/learn/FORGE-React-렌더링-파이프라인.md` | 판단 프레임 연결 | "리렌더링 = 느림"이 아니라 어느 단계가 실제 병목인지 구분해야 한다는 기존 판단 프레임과 연결됩니다. |
