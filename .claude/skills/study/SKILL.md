---
name: study
description: 소스코드 기반 딥 학습 — plan.md 기반 로드맵 → 설명+Q&A → 대화 기록
argument-hint: "<주제>"
disable-model-invocation: true
allowed-tools: Read, Grep, Glob, Bash, Write, mcp__study__context_resolve, mcp__study__config_get, mcp__study__session_getResumePoint, mcp__study__session_getSourcePaths, mcp__study__session_getSourceDigest, mcp__study__session_appendLog, mcp__study__review_saveMeta, mcp__study__progress_getPlan, mcp__study__progress_updateCheckbox
---
...
5. plan.md의 다음 미완료 토픽부터 서브토픽 → 마이크로서브토픽 단위로 아래 3개 프로토콜을 적용한다.
   ...
   **5-C. DISCOVERY 프로토콜** — 설명 중 발견된 연관 개념을 추적한다.
   ...
   - **종료 시 일괄 반영**: Step 6에서 축적된 발견 목록을 사용자에게 제시 → 승인 시 Write로 plan.md에 토픽 추가.
