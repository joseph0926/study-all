...
description: 프로젝트 소스 분석 후 {project}/.study/plan.md를 생성/갱신한다.
...
실행 순서:
1. `context.resolve(mode=project)` → studyDir, sourceDir 확인
2. `progress.getModuleMap` → 프로젝트 모듈 분석
3. `progress.getCoverageMap` → 커버리지 분석
