...
> **`/learn`과의 핵심 차이 — 프로젝트 소스는 정답이 아닌 학습 대상**:
> - `/learn`의 `ref/` 소스(React, Next.js 등)는 커뮤니티가 검증한 **ground truth**다.
> - `/project-learn`의 프로젝트 소스는 사용자가 직접 작성한 코드로, **버그·기술부채·불명확한 의도**가 있을 수 있다.
> - 따라서 이 커맨드는 코드를 **설명하되 평가도 함께** 한다: "이 코드는 이렇게 동작한다" + "이 접근의 장단점은 이렇다".
> - 의도가 불명확하면 AI가 추측하지 않고 **사용자에게 의도를 확인**한다.
...
### 코드 평가
- **의도 명확성**: {아래 3단계 중 하나}
  - ✅ 명확 — 네이밍/구조/주석에서 의도가 분명히 드러남
  - ⚠️ 추측 — 코드 동작에서 의도를 유추할 수 있으나 확실하지 않음 → 추측 근거 명시
  - 🔍 사용자 확인 필요 — 의도를 판단할 수 없음 → "이 코드의 의도가 {X}인가요?" 질문
- **패턴 적합성**: {일반적 패턴/관례와 비교 — 다르다면 왜 다른지, 의도적 선택일 수 있는지}
- **잠재 이슈**: {발견된 경우에만 — 버그, 엣지케이스, 성능, 보안 등}
...
- **비판적 시각**: 코드가 "무엇을 하는지" 설명한 뒤, "이 접근이 적절한지"도 평가합니다. 단, 평가는 근거(일반 패턴, 공식 문서, 성능 특성 등)와 함께 제시하며, 사용자의 의도적 선택일 수 있음을 존중합니다.
- **의도 불명확 시 질문**: 코드의 의도를 판단할 수 없으면 AI가 추측하지 않고 사용자에게 "이 코드의 의도가 {X}인가요?"라고 묻습니다.
...
- "왜 이렇게?" — 이 코드의 설계 의도 탐구 (의도 불명확 시 사용자와 함께 추론)
- "더 나은 방법?" — 대안 패턴과 현재 구현 비교
- "이슈 없어?" — 잠재적 버그/엣지케이스/성능 문제 분석
- "다음" — 다음 마이크로 스텝으로 진행
...
6. **비판적 동반자**: 코드를 있는 그대로 설명하되, 일반적 패턴/관례와 다른 부분이 보이면 "이 부분은 일반적으로 {X} 패턴을 쓰는데, 여기서는 {Y}로 구현되어 있습니다. 의도적 선택인가요?"처럼 비교 시각을 제공합니다. 비교 기준은 스킬 레퍼런스, 공식 문서, 일반적 관례 순으로 참조합니다.
7. **"왜 이렇게?" 응답**: 사용자가 설계 의도를 물으면, (1) 코드에서 드러나는 단서(네이밍, 주석, git blame 등)를 먼저 살피고, (2) 단서가 부족하면 가능한 의도를 2-3가지 제시한 뒤 사용자에게 확인합니다.
8. **"더 나은 방법?" 응답**: 대안을 제시할 때는 (1) 현재 구현의 장단점, (2) 대안의 장단점, (3) 트레이드오프를 구조적으로 비교합니다. 단순히 "이게 더 낫다"가 아니라 "이런 상황에서는 이게 유리하다"로 맥락 의존적 비교를 합니다.
9. **"이슈 없어?" 응답**: 잠재 이슈를 분석할 때는 (1) 엣지케이스, (2) 에러 핸들링, (3) 성능, (4) 보안 순으로 체크합니다. 발견된 이슈는 심각도(높음/중간/낮음)와 함께 제시합니다.
...
### 발견 사항
{세션 중 발견된 이슈/개선 기회/의도 불명확 항목을 기록합니다. 없으면 "없음"으로 기록.}
...
#### 세션 기록 구조 검증
- [ ] 필수 섹션 6개가 모두 존재하는가: `학습 로드맵`, `학습 요약`, `소스 코드 경로`, `Q&A 전체 기록`, `발견 사항`, `연결 토픽`
...
✅ 세션 기록 구조: 필수 섹션 6/6, Q&A 원문 보존 확인
...
- **프로젝트 소스 ≠ 정답**: `/learn`의 `ref/` 소스와 달리, 프로젝트 소스는 버그/기술부채/불명확한 의도가 있을 수 있습니다. 코드를 "설명"할 때와 "평가"할 때를 구분하고, 평가 시에는 근거(일반 패턴, 스킬 레퍼런스, 공식 문서)를 반드시 함께 제시합니다.
- **Graceful Degradation + 비교 기준 병렬 참조**: 설명의 소스는 plan.md > 프로젝트 소스 Grep > 프로젝트 문서 > 일반 지식 순으로 폴백합니다. 동시에, 코드 평가의 비교 기준으로 스킬 레퍼런스(`~/.claude/skills/`)와 일반적 관례를 병렬로 참조하여 "이 코드는 이렇게 구현되어 있고, 일반적으로는 이런 패턴을 쓴다"는 비교 시각을 제공합니다. 어떤 소스를 사용 중인지 항상 명시합니다.
- **의도 불명확 시 질문 우선**: 코드의 의도를 판단할 수 없으면 AI가 추측하지 않고 사용자에게 확인합니다. "이 코드의 의도가 {X}인가요?"
