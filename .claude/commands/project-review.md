---
description: "프로젝트 학습 기록 기반 복습 — AI 질문, 내가 답변"
argument-hint: "<project-path> [topic]"
---

# /project-review — 프로젝트 학습 기록 기반 적응형 복습 세션

## MCP Execution Mode (필수)

- 우선 호출: `context.resolve(mode=project)`, `review.getQueue`, `review.getMeta`
- 쓰기 호출: `review.recordResult`, `review.saveMeta`
- 규칙: 복습 대기열/간격 계산은 MCP를 사용하고 프롬프트는 질문/판정/피드백에 집중한다.

당신은 사용자의 프로젝트 학습 기록(`.study/`)과 복습 이력(`-meta.md`)을 기반으로 복습 질문을 **한 문제씩** 출제하는 튜터입니다.
사용자가 답변하면 학습 기록의 원문과 대조하여 피드백하고, 통과 여부를 판단합니다.

> **`/project-learn`과의 차이**: `/project-learn`은 새로운 개념을 프로젝트 소스 코드 기반으로 학습하는 세션.
> `/project-review`는 이미 학습한 내용을 반복 복습하여 내재화하는 세션.
...
6. **STUDY_DIR 존재 확인**:
   - 없으면: "학습 기록이 없습니다. `/project-study {PROJECT_PATH}` 또는 `/project-learn {PROJECT_PATH}`으로 먼저 학습하세요." 출력 후 종료.
...
`{STUDY_DIR}/{Topic-Name}.md` 파일을 찾습니다 (Title-Case, 하이픈 구분).
- 파일이 없으면: `{STUDY_DIR}/` 내 파일 목록을 표시하고 AskUserQuestion으로 선택.

### TOPIC이 없는 경우

`{STUDY_DIR}/` 내 모든 `.md` 파일(`plan.md`, `*-quiz.md`, `*-meta.md` 제외)을 스캔하고 AskUserQuestion으로 선택합니다:

질문: "어떤 토픽을 복습할까요?"
- header: "Topic"
- 옵션: 파일명 기반으로 동적 생성 (최대 4개)
  - 예: `Routing-System.md` → "Routing System"
...
5. **발견 사항** → 이슈/개선점 회상 질문 소스 (있을 때만)

### 3-2. 메타 데이터 로딩

`{STUDY_DIR}/{Topic-Name}-meta.md` 파일이 존재하면 로딩합니다.
...
- **단순 사실** (예: "이 컴포넌트는 서버 컴포넌트") → **L2까지**. L3/L4를 억지로 만들지 않는다.
- **설계 원리** (예: "미들웨어 체인 구조") → **L3까지** 자연스러움. L4는 연결 토픽이 있을 때만.
- **코드 흐름** (예: "API 라우트 처리 과정") → **L3까지** (코드 기록이 있으면). L4는 다른 흐름과의 연결이 학습 기록에 있을 때만.
...
| 유형 | 예시 |
|------|------|
| **개념 회상** | "이 프로젝트에서 라우팅 미들웨어가 하는 핵심 역할은?" |
| **비교** | "서버 컴포넌트와 클라이언트 컴포넌트의 데이터 페칭 차이는?" |
| **원리** | "이 상태 업데이트가 리렌더링을 트리거하는 이유는?" |
| **구현 위치** | "인증 미들웨어는 어떤 파일에 정의되어 있나요?" |
| **연결** | "API 라우트와 서버 액션의 관계는?" |
| **반사실** | "만약 이 캐싱 로직이 없다면 어떤 문제가 생기나요?" |
| **비유 역추적** | "학습 때 사용한 비유에서 '{X}'는 코드의 어떤 부분에 해당하나요?" |
| **이슈 회상** | "학습 중 이 모듈에서 발견된 BUG/IMPROVE 사항은?" (발견 사항 섹션이 있을 때만) |
| **의도 확인** | "이 코드의 의도가 불명확했던 이유는?" (발견 사항에 UNCLEAR 항목이 있을 때만) |
...
| 유형 | 예시 | 출제 조건 |
|------|------|----------|
| **코드 예측** | "이 핸들러에서 인증 실패 시 반환값은?" | Q&A에 해당 함수의 코드 스니펫이 인용되었을 때 |
| **빈칸 채우기** | "`middleware.ts`에서 `___` 조건이 참이면 리다이렉트" | Q&A에서 해당 코드 줄이 설명되었을 때 |
| **호출 추적** | "요청이 들어올 때 미들웨어 → 라우터 → 핸들러 순서는?" | Q&A에 호출 흐름 설명이 있을 때 |
| **디버깅** | "이 API 라우트에서 세션이 null이라면 어떤 상황?" | Q&A에 해당 필드의 의미가 설명되었을 때 |

#### 크로스토픽 연결 유형 (L4)

**출제 조건** (모두 충족해야 함):
1. 현재 토픽의 "연결 토픽" 섹션에 해당 연결이 **명시**되어 있다
2. 연결 대상 토픽의 학습 기록(`{STUDY_DIR}/{연결토픽}.md`)이 **존재**한다
...
> `/project-learn {PROJECT_PATH} {토픽}`으로 이 개념을 다시 학습할 수 있습니다.
...
```
> `/project-review {PROJECT_PATH} {같은 토픽}`으로 다시 복습할 수 있습니다.
> 재학습이 필요하면 `/project-learn {PROJECT_PATH} {토픽}`을 사용하세요.
```
...
`{STUDY_DIR}/{Topic-Name}-quiz.md`

- `{Topic-Name}`은 원본 학습 기록 파일명에서 `.md`를 제거한 것
- 예: `Routing-System.md` → `Routing-System-quiz.md`
...
1. 이 세션에서 Write한 `{STUDY_DIR}/{Topic-Name}-quiz.md` 파일을 다시 Read합니다.
2. 이 세션에서 Write한 `{STUDY_DIR}/{Topic-Name}-meta.md` 파일을 다시 Read합니다.
3. 아래 체크리스트를 검증합니다.
