...
> **study-aio와의 차이**: `/study-skill`은 소스 코드 딥스터디 → 스킬 레퍼런스 검증/개선이 목적입니다.
> `study-aio`는 개념 학습 효율화(Q&A/퀴즈/플래시카드)용입니다.
...
모든 토픽 완료 시 (또는 사용자가 요청 시):

1. `python scripts/skills_audit.py` 실행하여 메타데이터/링크 정합성 검증합니다.
2. 세션 변경 사항을 요약합니다:
