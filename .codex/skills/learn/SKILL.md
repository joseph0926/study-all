---
name: learn
description: 자유 Q&A 기반 학습 — 질문 → 근거 탐색(ref/웹/추론) → 답변 → 반복 → 문서화. Codex에서는 `$learn <질문>`으로 호출한다.
---

# learn

입력: `$learn <질문>` (예: `$learn Suspense는 어떠한 원리로 동작하나요?`)

실행 순서:

1. `mcp__study__context_resolve(mode=skill, skill=learn)`로 컨텍스트 확인
...
4. 아래 순서로 답변한다. (선행 조직자 → 상세 → 통합)
   - 4-A. **비유** — 핵심 개념마다 실생활 비유를 **먼저** 제시한다.
...
1. 전체 Q&A를 `study/learn/<주제명>.md`에 Write한다.
   - 포맷: 아래 템플릿을 따른다.
   - 원문 그대로 기록한다. 오탈자만 수정.
2. `mcp__study__session_appendLog(context, topic=<주제명>, content=<요약>)`로 세션 기록.
