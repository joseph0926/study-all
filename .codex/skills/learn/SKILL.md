---
name: learn
description: 즉석 Q&A 학습 — 궁금한 걸 질문하면 소스코드/웹 근거를 찾아 비유와 시각화로 답변하고, 정리 시 문서로 저장한다. "이거 왜 이래?", "X가 뭐야?", "알려줘", "질문 있어" 등 학습 질문 시 사용한다. 프로젝트 경로 지정 시 해당 프로젝트 코드 기반으로 답변한다. Codex에서는 `$learn [project-path] <질문>`으로 호출한다.
---
