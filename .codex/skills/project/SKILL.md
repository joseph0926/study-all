---
name: project
description: 프로젝트 소스 분석 → 개선점 도출 → 논의 → 수정 → 검증 → 문서화. Codex에서는 `$project <path> [area]`로 호출한다.
---

# project

입력: `$project <project-path> [area]` (예: `$project /path/to/project`, `$project /path/to/project error-handling`)
...
1. `mcp__study__progress_getModuleMap` + 핵심 파일 탐색
...
1. `mcp__study__progress_getPlan`으로 기존 계획 로드
...
종료(`>>정리` 또는 `>>끝`) 시:
1. `mcp__study__session_appendLog(via="via /project")` → `{project}/.study/{Area-Name}.md`에 기록
2. `mcp__study__progress_updateCheckbox` → `plan.md` 갱신
