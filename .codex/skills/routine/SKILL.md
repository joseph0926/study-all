---
name: routine
description: 메인 학습 루틴 — 탐색→심화→코딩→체크포인트→복습등록 5단계 파이프라인. "루틴 시작", "공부하자", "오늘 뭐 배울까", "학습 시작", "루틴" 등의 요청 시 사용한다. 프로젝트 경로를 지정하면 해당 프로젝트 소스 기반 학습 모드로 동작한다. Codex에서는 `$routine [project-path] [주제]`으로 호출한다.
---
